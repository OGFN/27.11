    ...
    public:
        ...
        auto FindFunctionBoundary(bool forward = false) -> Scanner
        {
            ...
            for (auto i = (forward ? 1 : -1); forward ? (i < 2048) : (i > -2048); forward ? i++ : i--)
            {
                ...
                if ((ASM::byteIsA(byte, ASM::MNEMONIC::RETN) && ASM::byteIsA(next, ASM::MNEMONIC::INT3)) || (ASM::byteIsA(byte, ASM::MNEMONIC::INT3) && ASM::byteIsA(next, ASM::MNEMONIC::INT3)))
                {
    ...
    class TrampolineHook
    {
        ...
        void WriteAbsoluteJump(void* jumpLocation, void* destination)
        {
            uint8_t absJumpInstructions[] = {
                ASM::Mnemonic("CMOVNS"), 0xBA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x41, 0xFF, 0xE2
            };
        ...
        uintptr_t PrepareRestore()
        {
            ...
            scanner.ScanFor({ 0x48, 0x83, 0xEC });
